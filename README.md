# OneScriptIntegrator
Эффективное взаимодействие между сценариями проекта. Внедрение глобальных переменных и расширенная передача параметров. Применимо к OneScript версии 1.х.
...
Функция МояПроцедура() Экспорт
	Пар = ОбщаяСтруктура.ИОС.АргументыСобытия.Параметр;
	Возврат Пар.Ключ1;
КонецФункции
```
...
* Посмотрите в справке раздел **Дополнения**. Это заметки по случаю моей "короткой" памяти.
* Класс **НаблюдательФайловойСистемы (FileSystemWatcher)** с обработчиками событий.
* Класс **Процесс (Process)** с обработчиками событий.
* Класс **Таймер (Timer)** с обработчиком событий.
* Класс **Математика (Math)**.
* Класс **Кодировка (Encoding)**.
